...
export function calculateInnerPanelHeight(panel: PanelModel, containerHeight: number): number {
  const chromePaddingHeight = panel.plugin && panel.plugin.noPadding ? 0 : config.theme.panelPadTop + config.theme.panelPadBottom; // Psiphon change
  const headerHeight = panel.hasTitle() ? config.theme.panelHeaderHeight : 0;
  return containerHeight - headerHeight - chromePaddingHeight - PANEL_BORDER; // Psiphon change
}
